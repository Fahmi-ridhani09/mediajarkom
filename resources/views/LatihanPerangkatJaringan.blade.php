...
<head>
    ...
    <style>
        ...
        .badge.bg-success {
            background-color: #b56f3c !important;
            /* badge jawaban */
        }
        ...
        .btn-primary:focus {
            ...
            border-color: var(--coklat-tua) !important;
        }
    ...
    <div class="container">
        <div class="row mt-3">
                ...
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title">Hasil latihan {{ $materi }}</h5>
            ...
            <script>
                ...
                let nilai = 0
                let answer = [];
                let startTimeRaw = "{{ session('startTime') }}";
                let endTimeRaw = "{{ session('endtime') }}";

                let startTime = new Date(startTimeRaw).getTime();
                let endTime = new Date(endTimeRaw).getTime();

                // fallback jika waktu invalid
                if (isNaN(startTime) || isNaN(endTime)) {
                    const now = new Date();
                    startTime = now.getTime();
                    endTime = now.getTime() + 20 * 60 * 1000; //20 menit dari sekarang
                }

                let remainingDuration = Math.floor((endTime - new Date().getTime()) / 1000);
                ...
                stopTest.addEventListener("click", () => {
                    swal.fire({
                        ...
                        if (willSubmit) {
                            window.location.href = "/perangkat_jaringan"
                        }
                    });
                })
                ...
                submitBtn.addEventListener('click', function() {
                    swal.fire({
                        ...
                        text: "Periksa kembali jawaban anda sebelum mengakhiri latihan",
                        icon: "warning",
                        showCancelButton: true,
        ...
        `
                    ...
                    footer += `<a href="/perangkat_jaringan"  class="btn btn-secondary">Kembali ke Materi!</a>`
                    if (nilai >= 70) {
                        footer += `  <a href="/materi/halamanmateri"  class="btn btn-primary">Materi Selanjutnya!</a>`
                    }
